...
def preprocess_sig(sig, frame_size):
    frames = FramedSignal(sig, frame_size = frame_size, fps = 344)
...
def preprocess_x(filename):
    sig = Signal(filename, sample_rate = 22050, num_channels = 1)
    D = [preprocess_sig(sig, fs) for fs in [1024, 512, 2048]]
    D = np.dstack(D)
